...
-- 2. For any rating where the reviewer is the same as the director of 
...
-- 5. For all pairs of reviewers such that both reviewers gave a 
...
-- 6. For each rating that is the lowest (fewest stars) currently in 
-- the database, return the reviewer name, movie title, and number of stars. 
...
SELECT title, avg(stars) AS average_star
FROM Movie
JOIN Rating USING(mId)
JOIN Reviewer USING(rId)
GROUP BY title
ORDER BY average_star DESC, title;
...
-- 11.For each director, return the director's name 
